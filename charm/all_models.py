...
class NSF_Autoreg_CNNcond(nn.Module):
        ...
        for jd in range(self.dim):
            ...
            if self.base_dist in ["gauss","halfgauss"]:
                if self.ngauss == 1:
                    ...
                    if isinstance(self.B, float) or isinstance(self.B, int):
                        sig = 0.5 * self.B * (1 + nn.Tanh()(sig_orig)) * 0.5
                    elif isinstance(self.B[0], float) or isinstance(self.B[0], int):
                        sig = 0.5 * 0.5 * (self.B[1] - self.B[0]) * (1 + nn.Tanh()(sig_orig)) * 0.5
                    else:
                        try:
                            sig = 0.5 * 0.5 * (self.B[jd][1] - self.B[jd][0]) * (1 + nn.Tanh()(sig_orig)) * 0.5
                        except:
                            sig = 0.5 * 0.5 * (self.B[jd-1][1] - self.B[jd-1][0]) * (1 + nn.Tanh()(sig_orig)) * 0.5
                else:
                ...
                if self.base_dist == 'weibull':
                    ...
                    if self.mu_pos:
                        # mu = torch.exp(mu)
                        mu = (1 + nn.Tanh()(mu)) / 2
                    # sig = ((1 + nn.Tanh()(alpha)) / 4)
                    sig = torch.exp(alpha)

                    # TESTING FIXING THESE VALUES
                    # mu = mu*0 + 0.2
                    # sig = sig*0 + 0.05
            ...
            if self.base_dist == 'gauss':
                if self.ngauss == 1:
                    logp_jd = -0.5 * np.log(2 * np.pi) - 0.5 * torch.log(sig**2) - 0.5 * (x - mu)**2 / sig**2
                else:
                    Li_all = torch.zeros(mu_all.shape[0])
                    Li_all = Li_all.to(device)
                    for i in range(self.ngauss):
    ...
    def inverse(self, cond_inp=None, mask=None):
        ...
        for jd in range(self.dim):
            ...
            if self.base_dist == 'gauss':
                if self.ngauss == 1:
                    x = mu + torch.randn(cond_inp_jd.shape[0], device=device) * sig
                else:
                    counts = torch.distributions.multinomial.Multinomial(total_count=1, probs=pw_all).sample()
                    counts = counts.to(device)
                    # loop over gaussians
                ...
                if self.ngauss == 1:
                    ...
                    # x = torch.log(mu + torch.abs(torch.randn(cond_inp_jd.shape[0], device=device)) * sig)         
                    hf = HalfNormal((sig))
                    x = hf.sample()
                    x += mu
