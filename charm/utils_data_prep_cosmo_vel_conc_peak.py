...
def get_subvol_selection(df_Mh_all, df_Nh_all, df_v_all, df_c_all, df_nu_all, nsubsel=None, ind_subsel=None, subsel_criteria='all', dtype=np.float16):
    # convert from sparse to dense:
    df_Mh_all, df_Nh_all, df_v_all, df_c_all, df_nu_all = df_Mh_all.todense().astype(dtype), df_Nh_all.todense().astype(dtype), df_v_all.todense().astype(dtype), df_c_all.todense().astype(dtype), df_nu_all.todense().astype(dtype)
    ...
    if subsel_criteria == 'ind':
        ...
        df_c_sel = df_c_all[ind_subsel, ...]
        df_nu_sel = df_nu_all[ind_subsel, ...]
    if subsel_criteria == 'highM1':
        ...
        df_c_sel = df_c_all[argsort_M1, ...]
        df_nu_sel = df_nu_all[argsort_M1, ...]
    ...
    if subsel_criteria == 'highNtot':
        ...
        df_c_sel = df_c_all[argsort_Ntot, ...]
        df_nu_sel = df_nu_all[argsort_Ntot, ...]
    ...
    if subsel_criteria == 'random':
        ...
        df_c_sel = df_c_all[arg_rand, ...]
        df_nu_sel = df_nu_all[arg_rand, ...]
        ind_subsel = arg_rand
    return df_Mh_sel, df_Nh_sel, df_v_sel, df_c_sel, df_nu_sel, ind_subsel
...
def load_density_halo_data_NGP(
    ...
    df_d_all, df_d_all_nsh, df_Mh_all, df_Nh_all, df_v_all, df_c_all, df_nu_all, cosmo_val_all = None, None, None, None, None, None, None, None
    ...
    if get_halos:
            ...
            for ji in tqdm(range(len(ji_array))):        
                ...
                if ji == 0:
                    if indsubsel_all_inp is None:
                        df_Mh_all, df_Nh_all, df_v_all, df_c_all, df_nu_all, ind_subsel_ji = get_subvol_selection(df_h['M_halos'], df_h['N_halos'], df_h[f'v_halos_{vel_type}'], df_h[f'c_halos_{conc_type}'], df_h['nu_halos'], nsubsel=nsubvol_per_ji, subsel_criteria=subsel_criteria, dtype=dtype_massNv)
                    else:
                        df_Mh_all, df_Nh_all, df_v_all, df_c_all, df_nu_all, ind_subsel_ji = get_subvol_selection(df_h['M_halos'], df_h['N_halos'], df_h[f'v_halos_{vel_type}'], df_h[f'c_halos_{conc_type}'], df_h['nu_halos'], ind_subsel=indsubsel_all_inp[jsim], subsel_criteria='ind', dtype=dtype_massNv)
                    cosmo_val_all = np.tile(LH_cosmo_val_all[jsim], (*df_Nh_all.shape ,1)).astype(dtype_massNv)
                else:
                    if indsubsel_all_inp is None:
                        df_Mh_all_ji, df_Nh_all_ji, dh_v_all_ji, df_c_all_ji, df_nu_all_ji, ind_subsel_ji = get_subvol_selection(df_h['M_halos'], df_h['N_halos'], df_h[f'v_halos_{vel_type}'], df_h[f'c_halos_{conc_type}'], df_h['nu_halos'], nsubsel=nsubvol_per_ji, subsel_criteria=subsel_criteria, dtype=dtype_massNv)
                    else:
                        df_Mh_all_ji, df_Nh_all_ji, dh_v_all_ji, df_c_all_ji, df_nu_all_ji, ind_subsel_ji = get_subvol_selection(df_h['M_halos'], df_h['N_halos'], df_h[f'v_halos_{vel_type}'], df_h[f'c_halos_{conc_type}'], df_h['nu_halos'], ind_subsel=indsubsel_all_inp[jsim], subsel_criteria='ind', dtype=dtype_massNv)
                    ...
                    df_c_all = np.concatenate((df_c_all, df_c_all_ji), axis=0)
                    df_nu_all = np.concatenate((df_nu_all, df_nu_all_ji), axis=0)
        ...
        if nsubvol_fid > 0:
            ...
            if indsubsel_fid_inp is None:
                df_Mh_all_ji, df_Nh_all_ji, dh_v_all_ji, df_c_all_ji, df_nu_all_ji, ind_subsel_ji = get_subvol_selection(df_h['M_halos'], df_h['N_halos'], df_h[f'v_halos_{vel_type}'],df_h[f'c_halos_{conc_type}'], df_h['nu_halos'],  nsubsel=nsubvol_fid, subsel_criteria=subsel_criteria, dtype=dtype_massNv)
            else:
                df_Mh_all_ji, df_Nh_all_ji, dh_v_all_ji, df_c_all_ji, df_nu_all_ji, ind_subsel_ji = get_subvol_selection(df_h['M_halos'], df_h['N_halos'], df_h[f'v_halos_{vel_type}'],df_h[f'c_halos_{conc_type}'], df_h['nu_halos'], ind_subsel=indsubsel_fid_inp, subsel_criteria='ind', dtype=dtype_massNv)
            if len(ji_array) > 0:
                df_Mh_all = np.concatenate((df_Mh_all, df_Mh_all_ji), axis=0)
                df_Nh_all = np.concatenate((df_Nh_all, df_Nh_all_ji), axis=0)
                df_v_all = np.concatenate((df_v_all, dh_v_all_ji), axis=0)
                ...
                df_c_all = df_c_all_ji
                df_nu_all = df_nu_all_ji
                cosmo_val_ji = (np.tile(fid_cosmo_val_all, (*df_Nh_all.shape ,1))[ind_subsel_ji,...]).astype(dtype_massNv)
    ...
    return df_d_all, df_d_all_nsh, df_Mh_all, df_Nh_all, df_v_all, df_c_all, df_nu_all, indsubsel_all, ind_subsel_fid, cosmo_val_all


def prep_density_halo_cats(
        df_d_all, df_d_all_nsh, df_Mh_all, df_Nh, cosmo_val_all, df_v_all=None, df_c_all=None, df_nu_all=None, nsims=None, nstart=None, Mmin=13.1, 
        Mmax=16.0, nuMax=5.0, rescaleM_sub=0.0, rescalenu_sub=0.0, Nmax=None, sigv=0.05, vmin=-1000, vmax=1000, cmin=-8, cmax=8
    ):
    if nstart is None:
        if nsims is None:
        ...
        df_c_all = df_c_all[:nsims, ...]
        df_nu_all = df_nu_all[:nsims, ...]
        ...
        df_c_all = df_c_all[nstart:nend, ...]
        df_nu_all = df_nu_all[nstart:nend, ...]
    ...
    c_halos_all = np.clip(c_halos_all, cmin, cmax)
    ...
    nu_halos_all_sort = np.take_along_axis(nu_halos_all, M_halos_argsort, axis=-1)
    v_halos_all_sort = np.take_along_axis(v_halos_all, M_halos_argsort, axis=-2)
    c_halos_all_sort = np.take_along_axis(c_halos_all, M_halos_argsort, axis=-1)

    M_halos_all_sort = M_halos_all_sort[...,:Nmax]   
    nu_halos_all_sort = nu_halos_all_sort[...,:Nmax] 
    v_halos_all_sort = v_halos_all_sort[...,:Nmax, :]
    c_halos_all_sort = c_halos_all_sort[...,:Nmax]
    ...
    nu_halos_all_sort_norm = rescalenu_sub + ((nu_halos_all_sort) / (nuMax))
    ind_rescalenu_sub = np.where(nu_halos_all_sort_norm < rescalenu_sub)
    nu_halos_all_sort_norm[ind_rescalenu_sub] = rescalenu_sub

    v_halos_all_sort_norm = (v_halos_all_sort - vmin) / (vmax - vmin)

    c_halos_all_sort_norm = (c_halos_all_sort - cmin) / (cmax - cmin)
    ...
    nu1_halos_all_norm = nu_halos_all_sort_norm[..., 0]
    ...
    nu_diff_halos_all_norm = nu_halos_all_sort_norm[..., :-1] - nu_halos_all_sort_norm[..., 1:]
    ...
    nu_diff_halos_all_norm_masked = nu_diff_halos_all_norm * mask_M_diff
    ...
    return_dict['nu_halos_all_sort_norm'] = np.array(nu_halos_all_sort_norm)
    return_dict['v_halos_all_sort_norm'] = np.array(v_halos_all_sort_norm)
    return_dict['c_halos_all_sort_norm'] = np.array(c_halos_all_sort_norm)
    ...
    return_dict['nu1_halos_all_norm'] = np.array(nu1_halos_all_norm)
    return_dict['nu_diff_halos_all_norm_masked'] = np.array(nu_diff_halos_all_norm_masked)
...
def prep_density_halo_cats_batched(
    ...
    nu_halos_all_sort_norm_all, nu1_halos_all_norm_all = [], []
    v_halos_all_sort_norm_all = []
    c_halos_all_sort_norm_all = []
    M_diff_halos_all_norm_masked_all = []
    nu_diff_halos_all_norm_masked_all = []
    v_halos_all_norm_masked_all = []
    c_halos_all_norm_masked_all = []
    ...
    for jb in range(nbatches):
        ...
        if get_halos:
            ...
            if df_v_inp is not None:
                ...
                v_halos_all = np.clip(v_halos_all, vmin, vmax)
            if df_c_inp is not None:
                c_halos_all = df_c_inp[nstart:nend, ...]
                c_halos_all = np.clip(c_halos_all, cmin, cmax)
            if df_nu_inp is not None:
                nu_halos_all = df_nu_inp[nstart:nend, ...]
                # nu_halos_all = np.clip(nu_halos_all, 0, nuMax)        
            # Now we reshape the number of halos into 2D array of shape number of sub-sim, nvoxels (per sub-sim)
            # Note that the number of sub-sim = nb**3
            N_halos_all = df_Nh.reshape((df_Nh.shape[0], df_Nh.shape[1] * df_Nh.shape[2] * df_Nh.shape[3]))
            if Nmax is None:
            ...
            if df_c_inp is not None:
                c_halos_all = c_halos_all.reshape(
                    (df_Mh_all.shape[0], df_Mh_all.shape[1] * df_Mh_all.shape[2] * df_Mh_all.shape[3], df_Mh_all.shape[4])
                    )

                # c_halos_argsort = np.tile(M_halos_argsort[..., np.newaxis], 1)
                c_halos_all_sort = np.take_along_axis(c_halos_all, M_halos_argsort, axis=-1)
                c_halos_all_sort = c_halos_all_sort[...,:Nmax]
                c_halos_all_sort_norm = (c_halos_all_sort - cmin) / (cmax - cmin)
                c_halos_all_sort_norm_all.append(c_halos_all_sort_norm)

            if df_nu_inp is not None:
                nu_halos_all = nu_halos_all.reshape(
                    (df_Mh_all.shape[0], df_Mh_all.shape[1] * df_Mh_all.shape[2] * df_Mh_all.shape[3], df_Mh_all.shape[4])
                    )

                # nu_halos_argsort = np.tile(M_halos_argsort[..., np.newaxis], 1)
                nu_halos_all_sort = np.take_along_axis(nu_halos_all, M_halos_argsort, axis=-1)
                nu_halos_all_sort = nu_halos_all_sort[...,:Nmax]
                nu_halos_all_sort_norm = rescalenu_sub + (nu_halos_all_sort) / (nuMax)
                nu_halos_all_sort_norm_all.append(nu_halos_all_sort_norm)
            ...
            if df_nu_inp is not None:
                nu1_halos_all_norm = nu_halos_all_sort_norm[..., 0]
                nu1_halos_all_norm_all.append(nu1_halos_all_norm)
            ...
            if df_nu_inp is not None:
                nu_diff_halos_all_norm = (nu_halos_all_sort_norm[..., :-1] - nu_halos_all_sort_norm[..., 1:])
                nu_diff_halos_all_norm_masked = (nu_diff_halos_all_norm * mask_M_diff)
                nu_diff_halos_all_norm_masked_all.append(nu_diff_halos_all_norm_masked)
            ...
            if df_c_inp is not None:
                c_halos_all_norm_masked = (c_halos_all_sort_norm * mask_all)
                c_halos_all_norm_masked_all.append(c_halos_all_norm_masked)
    ...
    return_dict['nu_halos_all_sort_norm'] = np.array(nu_halos_all_sort_norm_all)
    return_dict['v_halos_all_sort_norm'] = np.array(v_halos_all_sort_norm_all)
    return_dict['c_halos_all_sort_norm'] = np.array(c_halos_all_sort_norm_all)
    return_dict['Mmin'] = Mmin
    return_dict['Mmax'] = Mmax
    return_dict['Nmax'] = Nmax
    ...
    return_dict['nu1_halos_all_norm'] = np.array(nu1_halos_all_norm_all)
    return_dict['nu_diff_halos_all_norm_masked'] = np.array(nu_diff_halos_all_norm_masked_all)
    return_dict['v_halos_all_norm_masked'] = np.array(v_halos_all_norm_masked_all)
    return_dict['c_halos_all_norm_masked'] = np.array(c_halos_all_norm_masked_all)
