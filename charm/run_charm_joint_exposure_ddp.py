#!/usr/bin/env python
"""
run_charm_joint_exposure_ddp.py
-------------------------------
Exposure-robust DDP training runner for the unified CHARM model.  It keeps
the original teacher-forced objective, but scheduled downstream heads also see
the sampled upstream quantities they will receive at inference time.

Launch (single node, 4 GPUs):
    torchrun --standalone --nproc_per_node=4 charm/run_charm_joint_exposure_ddp.py \
...
Launch (multi-node, e.g. 2 nodes × 4 GPUs via SLURM):
    srun torchrun \
        ...
        charm/run_charm_joint_exposure_ddp.py --config run_configs/TRAIN_CHARM_JOINT.yaml
...
Key improvements over the three separate runners:
  ...
  - Scheduled sampled-upstream conditioning for M1/Mdiff/pos/vel/conc
  - Rollout validation with count, Ntot-PDF, and normalized-HMF diagnostics
"""
...
def setup_wandb(cfg: dict, rank: int, world_size: int, run_name: str = None,
    ...
    for prefix in ('train', 'val', 'rollout', 'perf', 'kendall', 'system', 'profile'):
...
def log_metrics(
    ...
    exposure_str = ''
    if student_prob is not None and student_prob > 0:
        exposure_str = f'  p_student={student_prob:.3f}'

    if not verbose and stdout:
        print(
            f'  step={global_step:6d}  ph={phase_idx}  '
            f'loss={display_total:.4f}(raw={raw_total:.4f})  '
            f'{head_str}{sigma_str}{exposure_str}  '
        ...
        print(
            ...
            f'│  heads: {head_str}{sigma_str}{exposure_str}\n'
    ...
    if student_prob is not None:
        log['train/exposure_student_prob'] = float(student_prob)
...
# ── Exposure-bias helpers ────────────────────────────────────────────────────

def exposure_defaults(cfg: dict) -> dict:
    """Return exposure-bias settings with conservative defaults filled in."""
    tc = cfg['train_settings']
    user = tc.get('exposure_bias', {}) or {}
    out = {
        'enabled':              False,
        'start_phase':          2,
        ...
        'rollout_val_every':    tc.get('val_every', tc.get('save_every', 100)),
...
def exposure_student_probability(exp_cfg: dict, phases: list,
                                 phase_idx: int, step_in_phase: int) -> float:
    """Linear schedule from student_prob_start to student_prob_end."""
    if not exp_cfg.get('enabled', False):
        return 0.0
    start_phase = int(exp_cfg.get('start_phase', 2))
    if phase_idx < start_phase:
        return 0.0

    total = sum(int(p['nepochs']) for p in phases[start_phase:])
    done = sum(int(p['nepochs']) for p in phases[start_phase:phase_idx])
    done += int(step_in_phase)
    frac = 0.0 if total <= 1 else min(1.0, max(0.0, done / float(total - 1)))

    p0 = float(exp_cfg.get('student_prob_start', 0.0))
    p1 = float(exp_cfg.get('student_prob_end', 0.7))
    floor = float(exp_cfg.get('teacher_loss_floor', 0.3))
    return max(0.0, min(1.0 - floor, p0 + frac * (p1 - p0)))
...
def _mean_abs_log_ratio(num: np.ndarray, den: np.ndarray,
    ...
    if not np.any(mask):
        return float('nan')
    return float(np.mean(np.abs(np.log((num[mask] + eps) / (den[mask] + eps)))))


def _sample_mass_matrix(sample_out: dict, nmax: int) -> np.ndarray:
    ntot = _as_np(sample_out['ntot'][0]).astype(np.float32)
    m1 = _as_np(sample_out['m1'][0]).astype(np.float32)
    mdiff = _as_np(sample_out['mdiff'][0]).astype(np.float32)
    m1 = np.clip(m1, 0.0, 1.0)
    mdiff = np.clip(mdiff, 0.0, 1.0)
    m = np.zeros((ntot.shape[0], nmax), dtype=np.float32)
    m[:, 0] = m1
    for j in range(1, nmax):
        m[:, j] = np.clip(m[:, j - 1] - mdiff[:, j - 1], 0.0, 1.0)
    slot = np.arange(nmax)[None, :] < ntot[:, None]
    return m * slot.astype(np.float32)


def _flatten_masses(m: np.ndarray, ntot: np.ndarray, nmax: int) -> np.ndarray:
    slot = np.arange(nmax)[None, :] < ntot[:, None]
    vals = np.asarray(m, dtype=np.float32)[slot]
    vals = vals[np.isfinite(vals)]
    return vals


def _sample_bound_rates(sample_out: dict) -> tuple:
    specs = [
        ('m1',    0.0, 1.0),
        ('mdiff', 0.0, 1.0),
        ('vel',  -0.5, 0.5),
    ...
    n_tot = 0
    n_nan = 0
    for key, lo, hi in specs:
        arr = _as_np(sample_out[key][0]).astype(np.float32)
        n_tot += arr.size
        finite = np.isfinite(arr)
        n_nan += int((~finite).sum())
        n_bad += int(((arr < lo) | (arr > hi) | (~finite)).sum())
    if n_tot == 0:
        return 0.0, 0.0
    return n_bad / n_tot, n_nan / n_tot


def _rollout_metrics(sample_out: dict, true_ntot: np.ndarray,
                     true_mnorm: np.ndarray, nmax: int) -> dict:
    pred_ntot = np.clip(np.rint(_as_np(sample_out['ntot'][0])), 0, nmax)
    true_ntot = np.clip(np.rint(true_ntot), 0, nmax)
    eps = 1e-6

    nocc_pred = float((pred_ntot > 0).sum())
    nocc_true = float((true_ntot > 0).sum())
    nh_pred = float(pred_ntot.sum())
    nh_true = float(true_ntot.sum())

    ntot_bins = np.arange(nmax + 2) - 0.5
    ntot_pred_hist, _ = np.histogram(pred_ntot, bins=ntot_bins)
    ntot_true_hist, _ = np.histogram(true_ntot, bins=ntot_bins)
    ntot_malr = _mean_abs_log_ratio(ntot_pred_hist, ntot_true_hist)

    pred_m = _sample_mass_matrix(sample_out, nmax)
    pred_vals = _flatten_masses(pred_m, pred_ntot, nmax)
    true_vals = _flatten_masses(true_mnorm, true_ntot, nmax)
    hmf_bins = np.linspace(0.0, 1.0, 21)
    hmf_pred, _ = np.histogram(pred_vals, bins=hmf_bins)
    hmf_true, _ = np.histogram(true_vals, bins=hmf_bins)
    hmf_malr = _mean_abs_log_ratio(hmf_pred, hmf_true)

    valid_pairs = np.arange(1, nmax)[None, :] < pred_ntot[:, None]
    if np.any(valid_pairs):
        monotonic_bad = pred_m[:, 1:] > pred_m[:, :-1] + 1e-6
        mono_rate = float((monotonic_bad & valid_pairs).sum() / valid_pairs.sum())
    else:
        mono_rate = 0.0

    bound_rate, nan_rate = _sample_bound_rates(sample_out)
    occ_ratio = (nocc_pred + eps) / (nocc_true + eps)
    halo_ratio = (nh_pred + eps) / (nh_true + eps)
    score = (
        abs(math.log(occ_ratio))
        + abs(math.log(halo_ratio))
        + (0.0 if not math.isfinite(hmf_malr) else hmf_malr)
        + 0.25 * (0.0 if not math.isfinite(ntot_malr) else ntot_malr)
    )
    return {
        'score': score,
        'occ_ratio': occ_ratio,
        'halo_ratio': halo_ratio,
        'ntot_pdf_malr': ntot_malr,
        'hmf_malr': hmf_malr,
        'mass_monotonic_violation_rate': mono_rate,
        'bound_violation_rate': bound_rate,
        'nan_rate': nan_rate,
    }


@torch.no_grad()
def run_rollout_validation(model: nn.Module, val_gpu: dict,
                           device: torch.device, cfg: dict,
                           max_outer: int = 4) -> dict:
    """Run free-sampling and oracle-ablation diagnostics on a small val slice."""
    raw = model.module if isinstance(model, DDP) else model
    raw.eval()

    sc = cfg['sim_settings']
    nmax = int(sc['Nmax'])
    n_outer = min(int(val_gpu['n_outer']), max(1, int(max_outer)))
    ...
    cosmo = val_gpu.get('cosmo')
    nhalos = N_halos.float().unsqueeze(-1)

    modes = {
        'free': {
            'sample_binary': True,  'sample_multi': True,
            'sample_m1': True,      'sample_mdiff': True,
            'use_truth_masses': False,
        },
        'truth_count': {
            'sample_binary': False, 'sample_multi': False,
            'sample_m1': True,      'sample_mdiff': True,
            'use_truth_masses': False,
        },
        'sample_count_truth_mass': {
            'sample_binary': True,  'sample_multi': True,
            'sample_m1': False,     'sample_mdiff': False,
            'use_truth_masses': True,
        },
    }
    accum = {mode: [] for mode in modes}

    with torch.amp.autocast(device_type='cuda', dtype=torch.bfloat16):
        for jb in range(n_outer):
            true_ntot = _as_np(N_halos[jb])
            true_mnorm = _as_np(mhalos[jb])
            btp = None
            if raw.binary_train_prior is not None:
                btp = float((N_halos[jb].float() > 0).float().mean().item())
                btp = min(1.0 - 1e-6, max(1e-6, btp))

            base = dict(
                cond_x=cond_x[jb:jb + 1],
                cond_x_nsh=cond_nsh[jb:jb + 1],
                cond_cosmo=cosmo[jb:jb + 1] if cosmo is not None else None,
            ...
            for mode, kwargs in modes.items():
                sample_out = raw.sample(**base, **kwargs)
                accum[mode].append(
                    _rollout_metrics(sample_out, true_ntot, true_mnorm, nmax)
                )
    ...
    for mode, vals in accum.items():
        keys = vals[0].keys()
        for key in keys:
            out[f'{mode}/{key}'] = float(np.nanmean([v[key] for v in vals]))
    out['score'] = out['free/score']
    raw.train()
    return out


def log_rollout_metrics(wb_run, metrics: dict, global_step: int,
                        phase_idx: int, best_score: float = None) -> None:
    score = metrics['score']
    delta = ''
    if best_score is not None and math.isfinite(best_score):
        d = score - best_score
        delta = f'  Δbest={"↓" if d < 0 else "↑"}{abs(d):.4f}'
    print(
        f'  [ROLLOUT] step={global_step:6d}  ph={phase_idx}  '
        f'score={score:.4f}{delta}  '
        f'occ={metrics["free/occ_ratio"]:.3f}  '
        f'nhalo={metrics["free/halo_ratio"]:.3f}  '
        f'hmf={metrics["free/hmf_malr"]:.3f}  '
        f'ntot={metrics["free/ntot_pdf_malr"]:.3f}',
        flush=True,
    )
    if wb_run is None:
        return
    log = {'global_step': global_step, 'rollout/phase': phase_idx}
    for k, v in metrics.items():
        log[f'rollout/{k.replace("/", "_")}'] = v
    if best_score is not None and math.isfinite(best_score):
        log['rollout/best_score'] = min(best_score, score)
    wb_run.log(log)
...
def save_best_val_checkpoint(
    ...
    path = os.path.join(ckpt_dir, 'checkpoint_best_teacher.pth')
    torch.save({
...
def save_best_rollout_checkpoint(
    ...
    """Save the best free-rollout checkpoint (rank 0 only, no barrier needed)."""
    raw = model.module if isinstance(model, DDP) else model
    ckpt_dir = cfg['train_settings']['checkpoint_dir']
    os.makedirs(ckpt_dir, exist_ok=True)
    path = os.path.join(ckpt_dir, 'checkpoint_best_rollout.pth')
    torch.save({
        'model_state':    raw.state_dict(),
        'encoder_state':  raw.encoder.state_dict(),
        'kendall_state':  kendall.state_dict() if kendall is not None else None,
        'global_step':    global_step,
        'phase_idx':      phase_idx,
        'rollout_score_min': rollout_score,
        'config':         cfg,
    }, path)
    print(f'  [rank0] Best-rollout checkpoint saved  score={rollout_score:.4f}  '
          f'→ {path}', flush=True)
...
def save_checkpoint(
    ...
    if rank == 0:
        ...
        payload = {
            ...
            'rollout_score_min': rollout_score_min,
            'config':         cfg,
        }
...
def load_checkpoint(
    ...
    rollout_score_min : float — best rollout score seen so far (inf for old ckpts)
    ...
    rollout_score_min = float(ckpt.get('rollout_score_min', float('inf')))
    optimizer_state = ckpt.get('optimizer_state', None)
    return (ckpt['global_step'], ckpt['phase_idx'], step_in_phase,
            ckpt['loss_min'], val_loss_min, rollout_score_min, optimizer_state)
...
def run_func(args):
    ...
    exp_cfg = exposure_defaults(cfg)
    if exp_cfg.get('enabled', False) and not exp_cfg.get('allow_torch_compile', False):
        # The exposure path contains stochastic sampled conditioning branches.
        # Keep it eager by default until those branches have been compiled and
        # profiled explicitly.
        tc['torch_compile'] = False

    if rank == 0:
        print(f'World size: {world_size}   Local rank: {local_rank}', flush=True)
        print(f'Staggered training: {staggered}', flush=True)
        print(f'Exposure-bias training: {exp_cfg}', flush=True)
    ...
    resume_rollout_score_min = float('inf')
    ...
    if resume_path:
        (global_step, start_phase, start_step_in_phase,
         loss_min, resume_val_loss_min, resume_rollout_score_min,
         resume_opt_state) = load_checkpoint(
        ...
        if rank == 0:
            print(f'Resumed from {resume_path}\n'
                  ...
                  f'  loss_min={loss_min:.4f}  '
                  f'val_loss_min={resume_val_loss_min:.4f}  '
                  f'rollout_score_min={resume_rollout_score_min:.4f}',
                  flush=True)
    ...
    rollout_every   = int(exp_cfg.get('rollout_val_every', val_every) or 0)
    ...
    rollout_score_min = resume_rollout_score_min
    ...
    with profiler:
     for phase_idx, phase_cfg in enumerate(phases):
        ...
        for _ in range(phase_start_step, nepochs_ph):
            ...
            student_prob = exposure_student_probability(
                exp_cfg, phases, phase_idx, step_in_phase
            )
            for pg in optimizer.param_groups:
                if pg['name'] == 'encoder':
            ...
            for jb in range(n_outer):
                ...
                with ddp_ctx, torch.amp.autocast(device_type='cuda', dtype=torch.bfloat16):
                    losses_jb = model(
                        ...
                        heads_to_train = active_heads,
                        exposure_student_prob = student_prob,
                        exposure_sample_repeats = exp_cfg.get('sample_repeats', 1),
                    )
                if kendall_active:
            ...
            do_rollout_val = (
                ...
                and rollout_every > 0
                and global_step % rollout_every == 0
            )
            if rank == 0 and val_gpu is not None and do_rollout_val:
                _hb('starting rollout validation pass', global_step)
                rollout_metrics = run_rollout_validation(
                    model, val_gpu, device, cfg,
                    max_outer=exp_cfg.get('rollout_val_max_outer', 4),
                )
                log_rollout_metrics(
                    wb_run, rollout_metrics, global_step, phase_idx,
                    best_score=rollout_score_min,
                )
                if (exp_cfg.get('save_best_rollout', True)
                        and rollout_metrics['score'] < rollout_score_min):
                    rollout_score_min = rollout_metrics['score']
                    save_best_rollout_checkpoint(
                        model, global_step, phase_idx,
                        rollout_score_min, cfg, kendall=kendall,
                    )
                _hb('rollout validation pass done', global_step)
            ...
            if dist.is_initialized() and (
                global_step % val_every == 0 or do_rollout_val
            ):
    ...
    if rank == 0:
        ...
        if val_gpu is not None:
            ...
            if exp_cfg.get('enabled', False):
                print(f'Best rollout score: {rollout_score_min:.4f}', flush=True)
        if wb_run is not None:
            wb_run.summary['best_train_loss'] = loss_min
            wb_run.summary['best_val_loss']   = val_loss_min
            if exp_cfg.get('enabled', False):
                wb_run.summary['best_rollout_score'] = rollout_score_min
            wandb.finish()
